...
Player.onConnect = function(socket,username,progress){
    ...
    socket.on('sendPmToServer',function(data){
       ...
       if(recipientSocket === null){
           socket.emit('addToChat',  data.username + '는 온라인이 아닙니다.')
       } else {
         recipientSocket.emit('addToChat',  player.username +'으로부터'+':'+ data.message)
            socket.emit('addToChat', data.username + '에게'+':' + data.message)
       }
...
Bullet = function(param){
    ...
    var super_update = self.update;
    self.update = function(){
        if(self.timer++ > 100)//총알 일정 거리 넘으면 삭제
            self.toRemove = true;
        super_update();
        for(var i in Player.list){
            var p = Player.list[i]
